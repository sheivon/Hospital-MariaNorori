...
window.ModalSaveState = {
  buttons(form) {
    const buttons = Array.from(form.querySelectorAll('button[type="submit"], [data-modal-save]'));
    const formId = form.id;
    if (formId) {
      document.querySelectorAll(`[data-modal-save][form="${formId}"]`).forEach(button => {
  ...
  begin(form) {
    ...
    this.buttons(form).forEach(button => {
  ...
  async finish(form, message, icon) {
    try {
      if (typeof window.swal === 'function') {
        await window.swal({ title: '', text: message, icon: icon });
      }
    } finally {
      if (!form) return;
