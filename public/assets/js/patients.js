...
static async list(filters = {}) {
        ...
        const url = '/api/patients_list.php' + (params ? '?' + params : '');
        return PatientsDataLayer.request(url);
    }
...
class PatientsView {
  ...
  static async loadPatients() {
    ...
    if (!tableBody) return;
    console.log('[PatientsView] loadPatients called');
    try {
      showLoadingOverlay();
      const result = await PatientsDataLayer.list();
      const rows = Array.isArray(result.data) ? result.data : [];
      console.log('[PatientsView] Got', rows.length, 'rows from API');
      tableBody.innerHTML = '';
      if (!rows.length) {
  ...
  static patientFormFields = ['id','first_name','last_name','cedula','dob','gender','phone','email','insurance_provider','insurance_policy_no','expediente_no','procedencia','father_name','mother_name','education_level','employer','address','notes','marital_status'];
  ...
  static async openEditModal(id){
    ...
    try {
      ...
      this.patientFormFields.forEach(f => {
        const el = document.getElementById(f);
        if (el) el.value = p[f] || '';
      });
      const titleEl = document.getElementById('patientModalLabel');
      if (titleEl) titleEl.textContent = window.i18n_t ? window.i18n_t('edit_patient') : 'Edit Patient';
  ...
  static async initList(){
    ...
    if (btnAddPatient) {
      btnAddPatient.addEventListener('click', () => this.openCreateModal());
    }
  }
  ...
  static resetPatientForm() {
    ...
    const titleEl = document.getElementById('patientModalLabel');
    if (titleEl) titleEl.textContent = window.i18n_t ? window.i18n_t('add_patient') : 'Add Patient';
  }
  ...
  static initForm(){
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (payload.success) {
          console.log('[PatientsView] Save successful, refreshing table...');
          await this.loadPatients();
          console.log('[PatientsView] Table refreshed, hiding modal...');
