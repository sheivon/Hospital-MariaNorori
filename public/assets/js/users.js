  ...
  async loadRoles(selected = 'user') {
    ...
    try {
      ...
      select.innerHTML = roles.map(role => {
        const value = UserView.escapeHtml(role.role);
        const accesstype = role.accesstype ? UserView.escapeHtml(role.accesstype) : '';
        return `<option value="${value}">${value}${accesstype ? ` (${accesstype})` : ''}</option>`;
      }).join('');
      ...
      const fallbackRoles = [
        { role: 'admin', accesstype: 'full' },
        { role: 'doctor', accesstype: 'clinical' },
        { role: 'user', accesstype: 'basic' }
      ];
      select.innerHTML = fallbackRoles.map(r => {
        const value = UserView.escapeHtml(r.role);
        return `<option value="${value}">${value} (${UserView.escapeHtml(r.accesstype)})</option>`;
      }).join('');
      select.value = selected;
      if (select.value !== selected) {
        select.selectedIndex = fallbackRoles.length - 1;
      }
    }
  }
...
class UserView {
  ...
  static bindActions() {
    ...
    this.modal.form.addEventListener('submit', async event => {
      ...
      if (window.ModalSaveState && !window.ModalSaveState.begin(this.modal.form)) return;
      ...
      try {
        ...
        this.modal.close();
        this.reloadTable();
        await window.ModalSaveState?.finish(this.modal.form, t('saved') || 'Saved', 'success');
      } catch (error) {
        this.modal.showError(error.message || 'Unable to save user');
        await window.ModalSaveState?.finish(this.modal.form, error.message || 'Unable to save user', 'error');
      }
    });
