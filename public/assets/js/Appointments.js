...
class AppointmentsView {
    ...
    static getModal() {
        if (!this.modalInstance) {
            ...
            if (el && typeof bootstrap !== 'undefined') {
                ...
                // After the modal finishes closing, refresh the table only
                // when a create or update succeeded, then defensively remove
                // any leftover backdrop / body
                //      lock â€” Bootstrap 5 sometimes leaves them behind
     ...
     * Safe to call repeatedly â€” destroys any existing DataTable first.
     */
     ...
     * Safe to call multiple times â€” re-renders the option list each time.
    ...
    static async loadPatientsForSelect() {
        ...
        try {
            ...
            const result = await PatientsDataLayer.list({ has_open_encounter: 1 });
        ...
        try {
            ...
            const optionsHtml = providers
                ...
                .map(u => {
                    ...
                    const roleSuffix = u.role ? ` â€” ${this.escapeHtml(u.role)}` : '';
    ...
    static async loadEncountersForPatient(patientId, selectedEncounterId = "") {
        const select = document.getElementById('appointmentCrudEncounter');
        if (!select) return;
        const requestId = ++this.encounterRequestId;

        const placeholder = window.i18n_t
            ? window.i18n_t('select_encounter')
            : '-- None --';

        select.innerHTML = `<option value="" data-i18n="select_encounter">${placeholder}</option>`;

        if (!patientId) return;

        try {
            const result = await AppointmentsDataLayer.request(
                '/api/encounters_list.php?patient_id=' + encodeURIComponent(patientId)
            );
            if (requestId !== this.encounterRequestId) return;

            const encounters = Array.isArray(result && result.data) ? result.data : [];
            const optionsHtml = encounters
                .filter(encounter => encounter && encounter.id != null)
                .map(encounter => {
                    const date = encounter.encounter_date || '';
                    const type = encounter.encounter_type || '';
                    const label = [date, type].filter(Boolean).join(' - ') || ('#' + encounter.id);
                    return `<option value="${this.escapeHtml(encounter.id)}">${this.escapeHtml(label)}</option>`;
                })
                .join('');

            select.innerHTML = `<option value="" data-i18n="select_encounter">${placeholder}</option>${optionsHtml}`;
            select.value = selectedEncounterId || '';
        } catch (err) {
            console.error('Failed to load encounters for appointment select:', err);
            if (requestId !== this.encounterRequestId) return;

            select.innerHTML = `<option value="" data-i18n="select_encounter">${placeholder}</option>`;
            const errorDiv = document.getElementById('appointmentCrudError');
            if (errorDiv) {
                errorDiv.textContent = err.message || 'Failed to load encounters';
                        ...
                        { extend: "pdf" },
                        {
                            ...
                            action: function () {
                                if (typeof window.triggerCustomPrint === 'function') {
                                    window.triggerCustomPrint('appointments');
                                    return;
                                }
                                window.open('/print.php?resource=appointments', '_blank');
                            }
                        }
                    ]
                }
        ...
        try {
            ...
            rows.forEach(a => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <div class="btn-group">
                            <button class="btn btn-primary btn-sm btn-edit" data-id="${this.escapeHtml(a.id)}" title="Edit appointment" aria-label="Edit appointment">
                                <i class="fa fa-edit"></i>
                            </button>
                            <button class="btn btn-danger btn-sm btn-delete" data-id="${this.escapeHtml(a.id)}" title="Delete appointment" aria-label="Delete appointment">
    ...
    static async openEditModal(id) {
        ...
        try {
            ...
            await this.loadEncountersForPatient(a.patient_id, a.encounter_id);
    ...
    static initForm() {
        ...
        const patientSelect = document.getElementById('appointmentCrudPatient');
        if (patientSelect) {
            patientSelect.addEventListener('change', () => {
                this.loadEncountersForPatient(patientSelect.value);
            });
        }
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                this.getModal()?.hide();
                form.reset();
                await window.ModalSaveState?.finish(form, window.i18n_t ? window.i18n_t('saved') : 'Saved', 'success');
                ...
                await window.ModalSaveState?.finish(form, err.message || (window.i18n_t ? window.i18n_t('save_failed') : 'Save failed'), 'error');
            }
        });
    }
