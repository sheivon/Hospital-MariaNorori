...
if (isset($filters['encountered']) && $filters['encountered'] !== '') {
            $encounterExists = 'EXISTS (SELECT 1 FROM encounters e WHERE e.patient_id = patients.id';
            if ($this->hasDeletedAtForTable('encounters')) {
                $encounterExists .= ' AND e.deleted_at IS NULL';
            }
            $encounterExists .= ')';
            $where[] = (int)$filters['encountered'] === 1 ? $encounterExists : 'NOT ' . $encounterExists;
        }

        if (isset($filters['has_open_encounter']) && $filters['has_open_encounter'] !== '') {
            $openEncounterExists = "EXISTS (SELECT 1 FROM encounters e WHERE e.patient_id = patients.id AND LOWER(e.status) = 'open'";
            if ($this->hasDeletedAtForTable('encounters')) {
                $openEncounterExists .= ' AND e.deleted_at IS NULL';
            }
            $openEncounterExists .= ')';
            $where[] = (int)$filters['has_open_encounter'] === 1 ? $openEncounterExists : 'NOT ' . $openEncounterExists;
        }
