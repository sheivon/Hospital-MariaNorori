...
class SetupRepository
{
    ...
    public function runSchemaFromFile(array $config, string $schemaFile): string
    {
        ...
        $pdo = $this->connectDatabase($config, /* allowMultiStatements */ true);
        $statements = $this->splitSqlStatements($sql);

        if (empty($statements)) {
            throw new RuntimeException('Schema file contained no executable statements.');
        }
        ...
        // Verify what actually landed in the database. Without this, "success"
        // can be a lie — e.g. if every statement silently failed.
        $tables = $this->listTables($pdo);
        $tableCount = count($tables);

        if ($executed === 0 && empty($tables)) {
            throw new RuntimeException(
                "Schema execution produced no statements and no tables.\n" . implode("\n", $errors)
            );
        }

        $summary = sprintf(
            "Schema executed: %d statement(s) applied, %d table(s) present in `%s`.",
            $executed,
            $tableCount,
            $config['DB_NAME']
        );
        ...
        if (!empty($errors)) {
            $summary .= "\n" . count($errors) . " statement(s) reported errors:\n" . implode("\n", $errors);
        }
     ...
     * - Honors single-quoted, double-quoted, and backtick identifiers so
     *   semicolons inside them are not treated as terminators.
    ...
    private function splitSqlStatements(string $sql): array
    {
        ...
        // Strip trailing inline-comment-only lines.
        $lines = preg_split("/\r\n|\n|\r/", $sql);
        $clean = [];
        foreach ($lines as $line) {
            $trim = ltrim($line);
            ...
            // Drop trailing line comments but keep the rest of the line.
            $pos = strpos($line, '--');
            if ($pos !== false) {
                $before = substr($line, 0, $pos);
                $quoted = false;
                $quote = '';
                for ($j = 0, $n = strlen($before); $j < $n; $j++) {
                    $c = $before[$j];
                    if ($quoted) {
    ...
    private function previewStatement(string $statement): string
    {
        ...
        return strlen($oneLine) > 160 ? substr($oneLine, 0, 157) . '...' : $oneLine;
    }
    ...
    public function createSampleData(array $config): array
    {
        ...
        foreach ($patients as $patient) {
            ...
            $insertDiag = $pdo->prepare('INSERT INTO diagnostics (patient_id, encounter_id, type, unit, room, icd10_code, description, status, severity, date, time, plan, weight, height, inss_no, created_by, created_at, updated_at) VALUES (:patient_id, NULL, :type, :unit, :room, :icd10, :description, :status, :severity, :date, :time, :plan, :weight, :height, :inss_no, NULL, NOW(), NOW())');
            $insertDiag->execute([
    ...
    private function connectDatabase(array $config, bool $allowMultiStatements = false): PDO
    {
        $this->validateDbIdentifier($config['DB_NAME']);
        $dsn = sprintf(
        ...
        if ($allowMultiStatements) {
            // Required so PDO::exec() can ship the full migrations/init.sql
            // (multiple DDL statements) in a single round-trip.
