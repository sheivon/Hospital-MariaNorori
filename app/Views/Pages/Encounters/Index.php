  ...
  <div class="card">
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-sm table-striped dt-container" id="encountersTable" width="100%">
          <thead>
            <tr>
              <th>#</th>
...
<?php include APP_ROOT . '/app/Views/Shared/Modals/encounter_modal.php'; ?>
...
const table = $('#encountersTable').DataTable({
      ...
      { extend: 'pdf', exportOptions: { columns: ':not(:last-child)' } },
      {
        ...
        action: function () {
          if (typeof window.triggerCustomPrint === 'function') {
            window.triggerCustomPrint('encounters');
            return;
          }
  ...
  if (encounterForm) {
    encounterForm.addEventListener('submit', async function(e){
      ...
      if (window.ModalSaveState && !window.ModalSaveState.begin(encounterForm)) return;
      ...
      try {
        ...
        table.ajax.reload(null, false);
        await window.ModalSaveState?.finish(encounterForm, t('encounter_saved') || 'Encounter created', 'success');
      } catch (err) {
        setEncounterAlert(err.message || t('error') || 'Error');
        await window.ModalSaveState?.finish(encounterForm, err.message || t('error') || 'Error', 'error');
      }
    });
  }
});
