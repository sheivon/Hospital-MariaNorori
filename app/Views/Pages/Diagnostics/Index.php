  ...
  <div class="card">
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-sm table-striped dt-container" id="diagnosticsTable">
          <thead>
            <tr>
              <th>#</th>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="diagForm" class="row g-3">
          ...
          <div class="col-md-6">
            <label for="diagUnit" class="form-label" data-i18n="diagnostics_unit">Unit</label>
            <input type="text" class="form-control" id="diagUnit" name="unit">
          </div>

          <div class="col-md-6">
            <label for="diagRoom" class="form-label" data-i18n="diagnostics_room">Room</label>
          ...
          <div class="col-md-6">
            <label for="diagTime" class="form-label" data-i18n="diagnostics_time">Time</label>
            <input type="time" class="form-control" id="diagTime" name="time">
          </div>

          <div class="col-md-6">
            <label for="diagInss" class="form-label" data-i18n="diagnostics_inss">INSS</label>
            <input type="text" class="form-control" id="diagInss" name="inss_no">
          </div>

          <div class="col-md-6">
            <label for="diagPlan" class="form-label" data-i18n="diagnostics_plan">Plan</label>
            <input type="text" class="form-control" id="diagPlan" name="plan">
          </div>

          <div class="col-md-6">
            <label for="diagWeight" class="form-label" data-i18n="diagnostics_weight">Weight (kg)</label>
            <input type="number" step="0.1" class="form-control" id="diagWeight" name="weight">
          </div>

          <div class="col-md-6">
            <label for="diagHeight" class="form-label" data-i18n="diagnostics_height">Height (cm)</label>
      ...
      <div class="modal-footer">
        ...
        <button id="btnSaveDiag" class="btn btn-primary" type="button" form="diagForm" data-modal-save data-i18n="save"><i class="fa-solid fa-save me-1"></i>Save</button>
      </div>
    </div>
  </div>
</div>
...
(function(){
  ...
  function resetForm(){
    ...
    clearErrors();
    // Reset modal title using data-i18n attribute
    document.getElementById('diagnosticModalTitle').setAttribute('data-i18n', 'diagnostics_add_btn');
    // Let the i18n system handle the translation
    if (window.i18n_t) {
      document.getElementById('diagnosticModalTitle').textContent = window.i18n_t('diagnostics_add_btn');
    }
  }
  ...
  async function loadDiagnostic(id){
    try {
      ...
      // Set modal title to "Edit" using data-i18n
      document.getElementById('diagnosticModalTitle').setAttribute('data-i18n', 'diagnostics_edit_btn');
      // Let the i18n system handle the translation
      if (window.i18n_t) {
        document.getElementById('diagnosticModalTitle').textContent = window.i18n_t('diagnostics_edit_btn');
      }
    } catch (e) {
  ...
  async function submitForm(e){
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        throw new Error(json.error || t('error') || 'Error');
      }
      initModal().hide();
      resetForm();
      loadDiagnostics();
      await window.ModalSaveState?.finish(form, t('saved') || 'Saved', 'success');
    } catch (e) {
      setError(t('error')||'Error');
      await window.ModalSaveState?.finish(form, e.message || t('error') || 'Error', 'error');
    }
  }
